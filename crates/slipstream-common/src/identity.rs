...
pub fn create_quic_config(
    ...
    config: &Config,
) -> Result<quinn::ClientConfig, SlipstreamError> {
    ...
    let cert_params = CertificateParams::new(vec!["solana".to_string()])
        .map_err(|e| SlipstreamError::CertError(e.to_string()))?;
    ...
    let mut client_crypto = rustls::ClientConfig::builder()
        ...
        .map_err(|e| SlipstreamError::ConfigError(e.to_string()))?;
    ...
    let quic_crypto = QuicClientConfig::try_from(client_crypto)
        .map_err(|e| SlipstreamError::ConfigError(format!("QUIC crypto config error: {}", e)))?;
    ...
    transport_config.max_idle_timeout(Some(
        quinn::IdleTimeout::try_from(config.quic_idle_timeout())
            .map_err(|e| SlipstreamError::ConfigError(format!("Invalid idle timeout: {}", e)))?,
    ));
...
fn solana_to_rcgen_keypair(solana_pair: &Keypair) -> Result<rcgen::KeyPair, SlipstreamError> {
    ...
    rcgen::KeyPair::try_from(pkcs8_bytes.as_slice())
        .map_err(|e| SlipstreamError::CertError(format!("Key conversion failed: {}", e)))
}

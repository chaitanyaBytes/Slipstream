...
use slipstream_common::{create_quic_config, Config, SlipstreamError};
...
impl QuicEngine {
    pub fn new(identity: &Keypair, config: &Config) -> Result<Self, SlipstreamError> {
    ...
    pub async fn send_transaction(
        ...
        let mut send_stream = connection
            .open_uni()
            .await
            .map_err(|e| SlipstreamError::StreamError(format!("Failed to open stream: {}", e)))?;
    ...
    async fn get_connection(&self, addr: SocketAddr) -> Result<Connection, SlipstreamError> {
        ...
        let connecting = self
            .endpoint
            .connect(addr, "solana")
            .map_err(|e| SlipstreamError::ConnectionError(format!("Connect failed: {}", e)))?;

...
impl Cartographer {
    ...
    pub async fn refresh_topology(&self) -> Result<(), SlipstreamError> {
        ...
        let nodes = self
            ...
            .get_cluster_nodes()
            .await
            .map_err(|e| SlipstreamError::RpcError(format!("Failed to fetch nodes: {}", e)))?;
    ...
    pub async fn update_schedule(&self) -> Result<(), SlipstreamError> {
        let epoch_info =
            self.rpc.get_epoch_info().await.map_err(|e| {
                SlipstreamError::RpcError(format!("Failed to get epoch info: {}", e))
            })?;
        ...
        if current_epoch > stored_epoch || stored_epoch == 0 {
            ...
            let schedule_data = self
                ...
                .await
                .map_err(|e| {
                    SlipstreamError::RpcError(format!("Failed to get leader schedule: {}", e))
                })?
                .ok_or(SlipstreamError::ScheduleUnavailable)?;
    ...
    pub async fn fetch_rpc_slot(&self) -> Result<u64, SlipstreamError> {
        let slot = self
            ...
            .get_slot()
            .await
            .map_err(|e| SlipstreamError::RpcError(format!("Failed to get slot: {}", e)))?;
